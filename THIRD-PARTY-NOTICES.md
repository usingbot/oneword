...
The Liberation notice in this pinned package is **not** the SIL Open Font License. Read its actual GPL and exception text, including font/source redistribution requirements, before redistributing those assets; embedding a font in a document does not by itself apply that license to the document. OneWord copies all four installed PDF.js Liberation Sans files unchanged. Their exact bytes match Debian `fonts-liberation 1:1.07.4-11`, whose source is Liberation 1.07.4 plus the Debian patch/build archive, not the original unpatched binary release. [Liberation source evidence](docs/LIBERATION-SOURCE.md) records every hash, the complete three-file source set and the proposed distribution arrangement. Status: **LIKELY CLOSED — NEEDS LEGAL REVIEW**; naming/trademark terms and actual source delivery remain unresolved. No public source offer or legal sufficiency is asserted.
