...
export function freezeDocument(doc: TextDocument): TextDocument {
  return Object.freeze({ ...doc, revisions: Object.freeze(doc.revisions.map(r => Object.freeze({ ...r }))) })
}
...
export async function readTextFile(file: File): Promise<string> {
  if (!/\.txt$/iu.test(file.name)) throw new Error('Chỉ mở tệp .txt. PDF chưa được hỗ trợ.')
